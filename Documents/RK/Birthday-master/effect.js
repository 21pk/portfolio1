...
$('document').ready(function(){
    ...
    $('#story').click(function(){
        ...
        // Current time is June 1, 2025. Set birthdayDate to sometime in the past to trigger revealBirthdayContent().
        const currentTime = new Date();
        const birthdayPassed = currentTime.getTime() >= new Date('June 1, 2025 00:00:00').getTime(); // Check against fixed birthday date

        if (!birthdayPassed) {
            // If the birthday is in the future, the message sequence should wait for it to be displayed.
            // This part might need more complex logic if the message is revealed *before* the countdown ends.
            // For now, assuming the countdown has finished when 'story' is clicked.
            // Adjust the birthdayDate in the $(window).load function for testing.
        }
        ...
        function msgLoop (index) {
            ...
            if(index < totalParagraphs){
                ...
                $("p:nth-child("+(index+1)+")").fadeIn('fast').delay(messageDisplayDuration).promise().done(function(){
                    // Fade out the current paragraph after the display duration
                    $("p:nth-child("+(index+1)+")").fadeOut('fast', function() {
                        // If there's a next paragraph, call msgLoop for it
                ...
                // Safety net: if initial index is out of bounds or no paragraphs
                $('.cake').fadeIn('fast');
                $('#surprise_trigger_button').fadeIn('slow');
            }
        }

        // Initially hide all paragraphs before starting the loop
        $('.message p').hide();
